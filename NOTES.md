...
**次回やること:**
1. カメラズーム解除→プレイヤー操作復帰の実装(`CameraBase::Update()`の`BossApearState::CameraZoom`のTODO部分、`GameScene.cpp`)。
2. `BossEnemy::Update()`の落下防止バグの具体的な修正内容をコードで再確認(記録時点で詳細未確認)。
3. 多段ヒット防止見送りに伴うダメージ値固定の反映確認。
4. ボスの死亡演出はまだ未着手。
