...
### 進捗（2026-08-06 BossEnemyのスキニング描画、VS_INPUTの入力レイアウト不一致が真因と判明・解決）

**「ボスを頂点シェーダで描画しようとすると失敗する」問題を、先生（学校の先生が書いたDxLib純正シェーダーを使うサンプルプロジェクト）のコードと比較しながら切り分けデバッグで解決した。**

- ボーン行列の持ち方を、旧来の`float4x4 boneMatrix[64]`(register b8)から、先生のプロジェクトに合わせた`float4 localWorldMatrix[162]`(1ボーンにつきfloat4×3行、register b3、DxLibが`g_LocalWorldMatrix`として自動供給する形式)に変更。スキニング計算も、行列を丸ごと加重合成してから掛けるのではなく、行(`skinRow0/1/2`)ごとに重み付き合成してから`dot`を取る形に書き換えた。
- 最初に`w`成分（`float4 pos : POSITION`が`input.pos`をそのまま`dot`に渡していた点）を疑い、先生と同じく`w`を明示的に`1.0f`にする形に直したが、見た目は変化せず外れ。
- 次に「スキニングを使わず`mul(pos, transpose(world))`だけで変換する」実験を実施 → **これでもボスが見えなくなった**。同じ式は`LightingVS.hlsl`(Rock/Player等)では正常に動いているため、計算式ではなく「シェーダーに届いている頂点データそのもの」がおかしいと判明。パレット（ボーン行列側）の疑いはここで晴れた。
- **真因**: `VS_INPUT`構造体が、DxLibのスキンメッシュ頂点フォーマット（先生のシェーダーで使われている構成）と1対1で一致していなかった（入力レイアウトのズレ）。具体的な差分は以下の通り。
  - 位置: `float4 pos` → `float3 pos`（wは自分で1.0fを補う）
  - UV: `float2 uv`のみ → `float4 TexCoords0`＋`float4 TexCoords1`（DxLibはメイン/サブUVをfloat4単位で渡してくる。使うのは`.xy`だけでTexCoords1は受け取るだけでOK）
  - 接線: `float3 tangent`のみ → `Tan`(TANGENT)に加えて`float3 bin : BINORMAL0`（**この`BINORMAL0`の追加が最後の決め手**。これが無い間はボスが描画されない/消える状態が続いた）
  - ボーン番号: `uint4` → 先生は`int4`（型まで含めて一致させる必要があった）
- `VS_INPUT`をこの構成に丸ごと合わせ、`main`内の参照箇所（`input.pos`→`float4(input.pos.xyz,1.0f)`、`input.uv`→`input.TexCoords0.xy`等）も直したところ、ボスのスキニング描画に成功。
- **教訓**: DxLibの独自シェーダーを使う場合、頂点シェーダーの計算ロジックが正しくても、`VS_INPUT`の型・順番・セマンティクスが実際の頂点バッファのレイアウトと1バイトでもズレると、頂点データの解釈自体が崩れて「棒立ちですらなく完全に消える/壊れる」という壊れ方をする。ロジックのデバッグで進展がない時は、動作実績のある公式シェーダーの`VS_INPUT`定義と全項目を突き合わせるのが近道。

**次回**: アニメーション自体（実際にモーションが反映されるか）はまだ確認できていない。上記「次回やること」のC++側実装（ボーン行列を`localWorldMatrix`用の定数バッファに詰める処理、`LightingManager`のスキニング分岐）に進む。

### 進捗（2026-08-06続き BossEnemyアニメーション未反映を解決・大発見「ボーン行列はDxLibが自動供給する」）

**「アニメーターを再生しているのに棒立ちのまま動かない」問題を切り分けデバッグで解決。長い迷走の末、真因は2つあった。**

- **迷走の記録**: レジスタ番号(b3↔b8)、`VS_INPUT`の型(`blendIndices`の`uint4`→`int4`)、`world`行列の掛け忘れ、スキニング計算式そのもの、`MV1SetUseOrigShader(true)`単体での再現実験、モデルスケールを縮小しての再現実験(縮小したら「パーツがバラバラに崩れた状態」で描画されることを発見。これにより「何も描画されない」は実は「頂点が壊れた位置に吹き飛んでカメラ外に出ていただけ」と判明)、`MV1GetTriangleListLocalWorldMatrix`(トライアングルリスト単位のボーン数=25を取得できたが、これを使っても改善せず)など、あらゆる角度からの切り分けを実施。
- **真因1（最重要な発見）: 独自シェーダー(`MV1SetUseOrigShader(true)`)を使っていても、ボーン行列はC++側で手動送信する必要がなく、DxLibが`MV1DrawModel`のたびに自動的に`register(b3)`の`LocalWorldMatrixBuffer`へ供給してくれる。** これは先生(学校の先生のサンプルプロジェクト、SampleTPSGame)のC++コードを調査して発覚した。`charaModel_`(スキニングモデル)の描画コードには、ボーン行列をシェーダーに送るコードが一切存在しなかった(ピクセルシェーダー用のb4〜b7のセットのみ)。つまり8/3の「独自シェーダーだとボーン行列は自動供給されない」という以前のNOTES.mdの記録は誤りだった(恐らく調査時に見た資料の説明を、頂点シェーダー全体を完全自作するケースと混同していたと思われる)。
  - 対処: `BossEnemy.h`の`BoneBuffer`構造体・`m_cBuffBone`・`m_pCBuffBoneData`、`BossEnemy::Draw()`内でボーン行列を計算して`SetShaderConstantBuffer`する一連の処理、`ShaderRegister::cbuffer_bone`は全部不要と判明(**次回、削除して整理すること**)。シェーダー側`SkinnedLightingVS.hlsl`の`LocalWorldMatrixBuffer`は`register(b3)`のままでよい(DxLibが自動供給する前提の場所なので、C++側から書き込まなければ問題ない)。
- **真因2（アニメーションが反映されない直接の原因）: `BossEnemy.cpp`のアイドルアニメーション名`idle_anim_name`が`L"A_mechISO_Walk_L"`(小文字m)となっていたが、モデルに実際に埋め込まれている名前は`A_MechISO_Walk_L`(大文字M)だった。** `MV1GetAnimIndex`は大文字小文字を区別するため、1文字違いで該当アニメーションが見つからず`animIndex = -1`になり、`m_animator.Play()`が実質何もしないまま(`m_currentAttachIdx < 0`で`Update()`が早期returnする)棒立ちが続いていた。`MV1GetAnimNum`/`MV1GetAnimName`で実際にモデルが持つアニメーション名を列挙してログ出力したことで発覚。
  - 対処: `idle_anim_name`を`L"A_MechISO_Walk_L"`に修正して解決、アニメーションが正常に反映されるようになった。
- **教訓**: ①DxLibのモデル描画に関する「自動でやってくれること／自分でやる必要があること」の境界は、公式ドキュメントの記述だけでなく、実際に動いている実例(先生のプロジェクトのC++側)で裏取りするのが一番確実。②`MV1GetAnimIndex`などの名前検索系APIは大文字小文字を区別するので、`-1`が返ってきたら真っ先に`MV1GetAnimNum`+`MV1GetAnimName`で実際の名前を列挙して確認すること。③「何も描画されない」ように見えても、モデルスケールを一時的に縮小してみると、実は頂点が壊れた位置に飛んでいるだけ(=計算ロジックの中身の問題)なのか、本当に何も描かれていない(=シェーダー適用や入力レイアウトなど、もっと手前の問題)なのかを切り分けられることがある。

**次回やること:**
1. 不要になったボーン行列関連のC++コード(`BoneBuffer`/`m_cBuffBone`/`m_pCBuffBoneData`/`ShaderRegister::cbuffer_bone`、`BossEnemy::Draw()`内の関連処理)を削除して整理する。
2. `model_scale`が実験で`{1.0f,1.0f,1.0f}`のままになっているので`{3.0f,3.0f,3.0f}`に戻す。
3. ボスの本格実装(移動・回転・雑魚敵出現・死亡・ビーム)に着手。まずユーザーはエフェクト(脚が海に浸かった瞬間の水しぶき等、Update()内のTODOコメント)から着手する予定。
4. 保留中だった`FloatingEnemy`だけが不自然に暗い問題の調査は、まだ再開時期未定のまま。

### 進捗（2026-08-06続き2 BossEnemyの脚の水しぶきエフェクト実装・「浸かった瞬間とズレて見える」問題を調査）

**脚が海に浸かった瞬間を検知して`Splash`エフェクト(Effekseer)を再生する処理を実装。実際にプレイすると「浸かった瞬間ではなく、Y座標がかなり沈んでから(-20〜-30あたりで)エフェクトが出ているように見える」という違和感が発生し、原因を調査した。**

- **判定ロジック自体は正しく動いていることをログで確認済み**: `m_prevLegPositions[i].y > water_y && m_currentLegPositions[i].y < water_y`の条件は、`OutputDebugString`で`leg番号`/`prev`/`cur`を毎回出力して裏取りした結果、境界(`water_y=0`)を正しくまたいだ瞬間にのみ発火していた。「-20あたりで発火して見える」のは、1フレームでの脚の移動量が大きい(歩行モーションの踏み込みタイミングで最大30程度動くことがある)ため、`0`ちょうどをまたぐタイミングを取りこぼし、次に判定できた時点ではもう沈み込んでいた、という理屈。
- 途中、同じ脚(`leg=5`等)が短時間に複数回発火する現象も見つかったが、これは6本脚のうち特定の脚(右側の脚:leg1,3,5)がそれぞれ2回、左側(leg0,2,4)が1回、という規則性があり、歩行の左右非対称な動きに起因すると考えられる(バグとは断定せず、一旦保留)。
- **真因(最終的に判明): ロジックの問題ではなく、Effekseerの`Splash`エフェクト素材自体のタイムライン冒頭に、見た目に何も起きない「空白の時間」が含まれていた。** そのため「判定は正しいタイミングで発火しているのに、目に見えるエフェクトの発生はワンテンポ遅れる」というズレが生じていた。
- **教訓**: 「ロジックの判定タイミングがズレているように見える」ときは、判定ロジック自体だけでなく、**再生しているアセット(エフェクト・アニメーション等)の素材側に built-in の遅延がないか**も疑うべき。今回はログで判定ロジックの正しさを先に裏取りできていたため、消去法でエフェクト素材側の問題だと早めに切り分けられた。

**次回**: `Splash`エフェクトのタイムライン冒頭の空白を削る/前倒しする対応(Effekseerエディタ側の作業、ユーザーが引き続き担当)。上記の「不要になったボーン行列関連のC++コード削除」「model_scaleを{3,3,3}に戻す」もまだ未対応のまま残っている。
