...
**次回やること:**
1. `DisabledShootState`の新規作成（`Shoot`フォルダ、`IShootState`継承、中身は空）。
2. `Player`に「4ステート全部を無効化ステートに切り替える」ための処理（メソッドか、演出開始時に`GameScene`側から4つ個別に`ChangeXxxState`を呼ぶか、設計はまだ未確定）を用意する。
3. `GameScene`に登場演出のステート/フラグと、「①落下+カメラ揺れ→②着地+カメラ揺れ→③ズーム→④解除」の順序を管理する仕組みを実装する。
4. ボスの「上空から落下してくる」動き（Y座標の初期値を高くしておき、着地位置まで降下させる等）は未設計。
5. 上記の「多段ヒット防止見送り」に伴うダメージ値固定の反映（未実装であれば）。
